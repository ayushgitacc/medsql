    ...
    <>
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: d ? "rgba(255, 255, 255, 0.43)" : "rgba(5, 8, 16, 0.56)", fontFamily: "'Roboto Mono', monospace" }}>{TABLES.length} tables · {TABLES.reduce((a,t)=>a+t.cols.length,0)} columns</div>
        </div>
        {onClose && (
        ...
        {TABLES.map((t, i) => (
          <div key={t.name} className="table-card" onClick={() => setActiveTable(activeTable === t.name ? null : t.name)}
            style={{
              ...
              border: activeTable === t.name ? `1.5px solid ${t.color}66` : d ? "1px solid rgba(255, 255, 255, 0.09)" : "1.5px solid rgba(0, 0, 0, 0.14)",
